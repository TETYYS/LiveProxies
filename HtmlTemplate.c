...
#define CONFIG_STRING(cfg, svar, var) if (config_setting_lookup_string(cfg, svar, (const char**)(&var)) == CONFIG_FALSE) { Log(LOG_LEVEL_ERROR, "Failed to lookup %s, using stock HTML...", svar); HtmlTemplateUseStock = true; return; }
				...
				if (strcmp(dir->d_name, files[x]) == 0) {
					...
					if (hFile != NULL) {
						...
						if (strcmp(dir->d_name, "iface.tmpl") == 0)
							HtmlTemplateParse(hFile, &HtmlTemplateProxies, &HtmlTemplateUProxiesSize);
						if (strcmp(dir->d_name, "ifaceu.tmpl") == 0)
							HtmlTemplateParse(hFile, &HtmlTemplateUProxies, &HtmlTemplateProxiesSize);
...
static void HtmlTemplateFindFirst(char *Contents, OUT HTML_TEMPLATE_COMPONENT **Component, OUT char **Offset)
{
	...
	*Component = NULL;
	...
	for (size_t x = 0;x < HtmlTemplateDynamicSize;x++) {
		...
		Log(LOG_LEVEL_DEBUG, "HtmlTemplateFindFirst: %s -> %p", HtmlTemplateDynamic[x].name, cur);
...
{
	*Components = *Components == NULL ? malloc(++(*CurrentSize) * sizeof(*Components)) : realloc(*Components, *CurrentSize * sizeof(*Components));
	...
	while (1) {
		...
		if (comp == NULL) {
			...
			if (offset != curEnd) {
				...
				strncpy(compStatic->content, string, offset - curEnd);
				HtmlTemplateComponentPush(Template, SizeRef, comp);
				curEnd = offset;
			}
			if (offset == curEnd) {
				...
				if (!TableInfo.inTable) {
					if (Info.currentPage->page == INTERFACE_PAGE_UPROXIES) {
						if (strcmp(Components[x]->name, "{T_UPROXIES_HEAD_1}") == 0)
							...
							evbuffer_add_reference(Buffer, "Type", 7, NULL, NULL);
						else if (strcmp(Components[x]->name, "{T_UPROXIES_HEAD_3}") == 0)
							evbuffer_add_reference(Buffer, "Currently checking", 7, NULL, NULL);
							...
							evbuffer_add_reference(Buffer, "Rechecking", 7, NULL, NULL);
						...
						case HTML_TEMPLATE_TABLE_UPROXIES: {
							...
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_1}") == 0) {
								char *ip = IPv6MapToString2(uproxy->ip); {
									evbuffer_add_printf(Buffer, "<td>%s:%d</td>", ip, uproxy->port);
								} free(ip);
							}
							...
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_5}") == 0)
								evbuffer_add_reference(Buffer, uproxy->associatedProxy != NULL ? "check" : "x", uproxy->associatedProxy != NULL ? 5 : 1);
							break;
						}
						case HTML_TEMPLATE_TABLE_PROXIES: {
							...
							if (strcmp(Components[x]->name, "{T_PROXIES_ITEM_1}") == 0) {
								char *ip = IPv6MapToString2(proxy->ip); {
									evbuffer_add_printf(Buffer, "<td>%s:%d</td>", ip, proxy->port);
								} free(ip);
							}
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_2}") == 0)
							...
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_6}") == 0)
								evbuffer_add_printf(Buffer, "%d", proxy->httpTimeoutMs);
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_7}") == 0) {
								char *time = FormatTime(proxy->liveSinceMs); {
									evbuffer_add_printf(Buffer, "%s", time);
								} free(time);
							}
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_8}") == 0) {
								char *time = FormatTime(proxy->lastCheckedMs); {
									evbuffer_add_printf(Buffer, "%s", time);
								} free(time);
							}
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_9}") == 0)
								evbuffer_add_printf(Buffer, "%d", proxy->retries);
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_10}") == 0)
								evbuffer_add_printf(Buffer, "%d", proxy->successfulChecks);
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_11}") == 0)
								evbuffer_add_printf(Buffer, "%d", proxy->failedChecks);
							if (strcmp(Components[x]->name, "{T_UPROXIES_ITEM_12}") == 0) {
								...
								*((PROXY_TYPE*)(sid + IPV6_SIZE + sizeof(uint16_t))) = proxy->type;
								...
								Base64Encode(sid, IPV6_SIZE + sizeof(uint16_t) + sizeof(PROXY_TYPE), &sidb64); {
									evbuffer_add_printf(Buffer, "<td><a href=\"/iface/check?sid=%s\">Check</a></td>", sidb64);
								} free(sidb64);
							}
							break;
						}
				...
				if (strcmp(Components[x]->name, "{T_UPROXIES_TABLE_ITEMS_START}") == 0) {
					pthread_mutex_lock(&lockUncheckedProxies); {
						for (size_t i = 0;i < sizeUncheckedProxies;i++) {
							...
							tableInfo.tableObject = uncheckedProxies[x];
				...
				if (strcmp(Components[x]->name, "{T_PROXIES_TABLE_ITEMS_START}") == 0) {
					pthread_mutex_lock(&lockCheckedProxies); {
						for (size_t i = 0;i < sizeCheckedProxies;i++) {
							...
							tableInfo.tableObject = checkedProxies[x];
							HtmlTemplateBufferInsert(Buffer, Components, Size, Info, tableInfo);
						}
